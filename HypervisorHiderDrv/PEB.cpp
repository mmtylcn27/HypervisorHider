...
ULONG Hypervisor::PE::GetExportOffset(const unsigned char* FileData, ULONG FileSize, const char* ExportName)
{
    //Verify DOS Header
    ...
    if (ExportOffset == PE_ERROR_VALUE)
        DBGPRINT("Export %s not found in export table!", ExportName);

    return ExportOffset;
}

PVOID Hypervisor::PE::GetPageBase(PVOID lpHeader, ULONG* Size, PVOID ptr)
{
